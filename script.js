...
/* CONFIG: default API base (your Render app) */
const DEFAULT_API_BASE = 'https://ecommerce-crud-app.onrender.com';
let API = {
  base: localStorage.getItem('apiBase') || DEFAULT_API_BASE,
  products(){ return `${this.base.replace(/\/$/,'')}/api/products`; },
  login(){ return `${this.base.replace(/\/$/,'')}/api/auth/login`; },
  register(){ return `${this.base.replace(/\/$/,'')}/api/auth/register`; }
};

/* ADMIN credentials used for local fallback (not stored server-side here)
   (you already provided these) */
...
/* Ensure demo users + products present locally for fallback */
(function ensureLocalData(){
  try {
    if(!localStorage.getItem('localUsers')){
      ...
      const u = JSON.parse(localStorage.getItem('localUsers'));
      if(!u.find(x=>x.email===ADMIN.email)){ u.push({ id: 'admin-local', name: ADMIN.name, email: ADMIN.email, password: ADMIN.password }); localStorage.setItem('localUsers', JSON.stringify(u)); }
    }
  } catch(e){
    localStorage.setItem('localUsers', JSON.stringify([{ id: 'admin-local', name: ADMIN.name, email: ADMIN.email, password: ADMIN.password }]));
  }
...
async function apiCreateProduct(p){
  try {
    ...
    cur.unshift(item); localStorage.setItem('products', JSON.stringify(cur)); return item;
  }
}
...
async function apiDeleteProduct(id){
  try {
    ...
    localStorage.setItem('products', JSON.stringify(cur)); return true;
  }
}
...
function renderProducts(){
  ...
  grid.innerHTML = list.map(p=>`
    <div class="card p-4 rounded-lg flex flex-col">
      <div class="h-44 overflow-hidden mb-3">${p.image ? `<img src="${p.image}" alt="${escapeHtml(p.name)}" class="product-img w-full h-full object-cover rounded">` : `<div class="img-fallback h-44 rounded">No image</div>`}</div>
      <div class="flex-grow">
...
function renderAdminList(){
  const box = el('#adminProductList'); if(!box) return;
...
function renderCart(){
  const wrap = el('#cartDrawerContent'); const count = el('#cartCount'); const subtotalEl = el('#cartSubtotal');
...
function addToCart(id){
  const p = state.products.find(x=>x._id===id); if(!p) return;
...
function changeQty(id, delta){
  const it = state.cart.find(x=>x._id===id); if(!it) return;
  it.qty += delta; if(it.qty <= 0) state.cart = state.cart.filter(x => x._id !== id);
  localStorage.setItem('cart', JSON.stringify(state.cart)); renderCart();
}

/* Drawer & modal helpers */
function openCart(){ const layer = el('#cartDrawer'), aside = layer?.querySelector('aside'); if(!layer) return; layer.classList.remove('hidden'); aside.classList.remove('drawer-closed'); aside.classList.add('drawer-open'); }
function closeCart(){ const layer = el('#cartDrawer'), aside = layer?.querySelector('aside'); if(!layer) return; aside.classList.remove('drawer-open'); aside.classList.add('drawer-closed'); setTimeout(()=> layer.classList.add('hidden'), 260); }
...
function updateAccountUI(){
  ...
  if(state.user && (state.user.email === ADMIN.email || state.user.admin)) adminPanel?.classList.remove('hidden'); else adminPanel?.classList.add('hidden');
}
...
async function loginFlow(email, password){
  try {
    ...
    toast(err.message || 'Login failed'); return false;
  }
}
...
async function registerFlow(name, email, password){
  try {
    ...
    toast(err.message || 'Register failed'); return false;
  }
}
...
function loadProductIntoForm(id){
  const p = state.products.find(x=>x._id===id); if(!p) return;
  el('#prodId').value = p._id; el('#prodName').value = p.name; el('#prodCategory').value = p.category;
  el('#prodPrice').value = p.price; el('#prodStock').value = p.stock || 0; el('#prodImage').value = p.image || '';
  el('#prodDesc').value = p.description || ''; window.scrollTo({ top: el('#shop').offsetTop - 40, behavior:'smooth' });
}

/* Load products from API or local */
async function loadProducts(){
  const data = await apiGetProducts();
  state.products = (Array.isArray(data) ? data : []).map(p => ({ ...p, _id: p._id || p.id || 'p-'+uid() }));
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // populate API label if present
  // account menu injection
  (function injectAccountMenu(){
    ...
    wrapper.innerHTML = `
      ...
      <div id="apiSettings" class="mt-2 hidden">
        <input id="apiBaseInput" class="w-full px-2 py-1 text-xs mt-2 bg-black/10 rounded" placeholder="https://your-api.com" />
        <div class="mt-2 flex gap-2">
          <button id="saveApiBtn" class="px-2 py-1 rounded border text-xs">Save</button>
          <button id="testApiBtn" class="px-2 py-1 rounded border text-xs">Test</button>
        </div>
        <div id="apiTestResult" class="text-xs mt-2 text-slate-300"></div>
      </div>
      <div class="mt-2 flex gap-2">
  ...
  if(!sessionStorage.getItem('visited')){ openModal('#welcomeModal'); sessionStorage.setItem('visited','1'); }
  ...
  el('#openAuthBtnSmall')?.addEventListener('click', ()=> openModal('#authModal'));
  ...
  // auth modal toggles
  el('#switchToRegister')?.addEventListener('click', ()=> { el('#loginForm').classList.add('hidden'); el('#registerForm').classList.remove('hidden'); el('#authTitle').textContent = 'Register'; });
  el('#switchToLogin')?.addEventListener('click', ()=> { el('#registerForm').classList.add('hidden'); el('#loginForm').classList.remove('hidden'); el('#authTitle').textContent = 'Login'; });
  ...
  el('#registerForm')?.addEventListener('submit', async (e)=> {
    e.preventDefault();
    const name = el('#regName').value.trim(), email = el('#regEmail').value.trim(), password = el('#regPassword').value.trim();
  ...
  el('#productForm')?.addEventListener('submit', async (e)=> {
    ...
    if(id){ await apiUpdateProduct(id, payload); toast('Product updated'); }
    else { await apiCreateProduct(payload); toast('Product created'); }
    el('#productForm').reset(); await loadProducts();
  });
  el('#resetProductBtn')?.addEventListener('click', ()=> el('#productForm').reset());

  // account menu actions delegated
  document.body.addEventListener('click', async (e)=> {
    if(e.target.matches('[data-del]')){ const id = e.target.dataset.del; if(confirm('Delete?')){ await apiDeleteProduct(id); await loadProducts(); toast('Deleted'); } }
    if(e.target.matches('[data-edit]')){ loadProductIntoForm(e.target.dataset.edit); }
  });

  // API settings
  el('#saveApiBtn')?.addEventListener('click', ()=> {
    const val = el('#apiBaseInput').value.trim();
    if(!val) return toast('Provide API base');
    API.base = val.replace(/\/$/,''); localStorage.setItem('apiBase', API.base); toast('API base saved');
    el('#apiBaseLabel') && (el('#apiBaseLabel').textContent = API.base);
  });
  el('#testApiBtn')?.addEventListener('click', async ()=> {
    ...
    try { const res = await fetch(url); el('#apiTestResult').textContent = `GET /api/products → ${res.status} ${res.ok ? 'OK' : res.statusText}`; } catch(err) { el('#apiTestResult').textContent = `Failed to reach ${url}`; }
  });

  // If saved user exists, restore admin state
